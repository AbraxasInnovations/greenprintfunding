...
def get_token_descriptions():
    """
    Return a dictionary of token symbols to their full names
    """
    return {
        "BTC": "Bitcoin",
        "ETH": "Ethereum",
        "DOGE": "Dogecoin",
        "HPOS": "Harry Potter OSI10",
        "XLM": "Stellar Lumens",
        "ADA": "Cardano",
        "DOT": "Polkadot",
        "SOL": "Solana",
...
def create_token_dictionary():
    ...
    token_descriptions = get_token_descriptions()
    ...
    for coin in common_coins:
        ...
        description = token_descriptions.get(coin, f"{coin} token")
        
        # Create the token entry
        tokens[coin] = {
            "description": description,  # Full name of the token
...
def format_token_dictionary(token_dict):
    ...
    for token, data in sorted(token_dict.items()):
        ...
        output += f'        "description": "{data["description"]}",  # Full name of the token\n'
